...
delta_T_values, n_values, t50_values, tVar_values, distance_values, zenith_values, signal_values, group_values, counter = delta_t(events, 450)            
a, b, d = 177, 1.5, 64
...
t50_mean = np.array([arr.mean() for arr in t50_values])
n_mean = np.array([arr.mean() for arr in n_values])
distance_mean = np.array([arr.mean() for arr in distance_values])
delta_t = np.repeat(np.array(delta_T_values), 2)
print("t50: ", t50_values[:7], "\n")
print("n: ", n_values[:7], "\n")
print("delta_t: ", delta_T_values[:7], "\n\n\n")
print("t50: ", t50_mean[:7], "\n")
print("n: ", n_mean[:7], "\n")
print("delta_t: ", delta_t[:7], "\n")
X = np.column_stack((t50_mean, n_mean, distance_mean))
y = delta_T_values
print("X: ", X[:7], "\n")
print("y: ", y[:7], "\n")
# X, y = X[:10000], y[:10000]
...
function negative_log_likelihood(tree, dataset::Dataset{T,L}, options)::L where {T,L}
    ...
    nll = sum(
        (dataset.y .^ 2) ./ (prediction .+ ε) .+ 
        log.(2 .* π .* prediction .+ ε)
    )
    return nll
end
"""
...
print("5 variables single no input 500 iterations")
