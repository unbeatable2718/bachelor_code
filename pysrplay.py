...
# # print("t50: ", t50_flat[:7], "\n")
# # print("n: ", n_flat[:7], "\n")
# # print("delta_t: ", delta_t[:7], "\n")
# X = np.column_stack((t50_flat, n_flat, distance_flat))
...
kde = gaussian_kde(distance_flat)  # Kernel Density Estimation
density = kde(distance_flat)  # Evaluate density for each sample

# Compute weights (inverse density)
ε = 1e-8  # Small value to avoid division by zero
weights = 1 / (density + ε)
...
model = PySRRegressor(
    ...
    loss_function=weights*function_likeli,
    niterations=100,
    binary_operators=["+", "-", "*", "/", "^"],
