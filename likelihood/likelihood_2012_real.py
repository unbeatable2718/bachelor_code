...
delta_t_values, n_values, t50_values, tVar_values, distance_values, theta_values, signal_values, group_values, counter = delta_t(events, 450)
...
def V_t0(a2, b2, T_50, n):
    return a2 * pow(2 * T_50 / (n - 1), 2) * n / (n + 2) + b2

# Funktion für V[Delta_T]
def V_Delta_T(a2, b2, T_50_i, T_50_j, n_i, n_j):
    return V_t0(a2, b2, T_50_i, n_i) + V_t0(a2, b2, T_50_j, n_j)

def log_likelihood(a, b, c, d, e, f, delta_T, T_50_values, n_values, theta):
    ...
    for i in range(len(delta_T)):
        ...
        a2 = a + cos_theta * (b - c * cos_theta)
        b2 = (d + cos_theta * (e * cos_theta - f))
        # Berechne V[ΔT_i] als Summe von Varianzen
        V_delta_T_i = V_Delta_T(a2, b2, T_50_i, T_50_j, n_i, n_j)
        if V_delta_T_i <= 0:
            return np.inf
...
initial_params = [0.64871, 0.22365, 0.49971, 141.24, 412.19, 208.9]
#bounds = [(1, 500), (None, None), (-50, 50)]

minuit = Minuit(
    lambda a, b, c, d, e, f: log_likelihood(a, b, c, d, e, f, delta_T, T_50_values, n_values, theta_values),
    a=initial_params[0],
    b=initial_params[1],
    c=initial_params[2],
    d=initial_params[3],
