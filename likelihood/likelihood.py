...
# # Berechne die mittleren Distanzen aus den gespeicherten Werten
# flattened_distances = np.array([item for sublist in distance_values for item in sublist])
# # Definiere Bins für die Gewichtung
# num_bins = 25  # Anzahl der Bins (kann angepasst werden)
# hist, bin_edges = np.histogram(flattened_distances, bins=num_bins, density=False)

# # Verhindere Division durch Null
# hist = np.where(hist == 0, 1, hist)  

# # Berechne die inversen Gewichte
# weights = 1 / hist  

# # Weise jedem Datenpunkt das richtige Gewicht basierend auf seinem Abstand zu
# bin_indices = np.digitize(flattened_distances, bin_edges, right=True) - 1
# bin_indices = np.clip(bin_indices, 0, num_bins - 1)  # Falls Werte außerhalb des Intervalls sind
# point_weights = weights[bin_indices]

# # Modifizierte Log-Likelihood-Funktion mit Gewichtung
# def log_likelihood_weighted(a, b, d, delta_T, T_50_values, n_values, weights):
#     log_likelihood_sum = 0
#     for i in range(len(delta_T)):
#         T_50_i, T_50_j = T_50_values[i]
#         n_i, n_j = n_values[i]
#         weight = weights[i]  # Gewicht des Punktes
...
#         log_likelihood_sum += weight * (term1 + term2)  # Gewichtete Summe

#     return log_likelihood_sum

# # Starte die minimierte Fit-Berechnung mit der neuen gewichteten Funktion

# initial_params = [177, 1.5, 64]
# bounds = [(1, 500), (None, None), (-50, 50)]

# minuit = Minuit(
#     lambda a, b, d: log_likelihood_weighted(a, b, d, delta_T, T_50_values, n_values, point_weights),
#     a=initial_params[0],
#     b=initial_params[1],
#     d=initial_params[2]
# )
...
# print("Optimale Parameter mit Gewichtung:")
...
#         return log_likelihood_sum
#     print("paper", log_likelihood(134, 2.4, 10, delta_T, T_50_values, n_values))
...
#     print(minuit.fmin)
#     print(134, 2.4, 10, delta_T, T_50_values, n_values)
...
axs[0, 0].hist(delta_T, bins=80, histtype="step", color="blue", alpha=1)
axs[0, 0].set_title(r"$\Delta T$")
axs[0, 0].set_xlabel(r"$\Delta T /$ ns")
axs[0, 0].set_ylabel(r"Count")

axs[0, 1].hist(flattenedT50, bins=80, histtype="step", color="blue", alpha=1)
axs[0, 1].set_title(r"$T_{50}$")
axs[0, 1].set_xlabel(r"$T_{50} /$ ns")
axs[0, 1].set_ylabel(r"Count")

# Logarithmic x-axis for `n`
axs[1, 0].hist(flattenedn, bins=80, histtype="step", color="blue", alpha=1)
...
axs[1, 1].hist(flattenedSig, bins=80, histtype="step", color="blue", alpha=1)
axs[1, 1].set_title(r"Signal (Logarithmic X-Scale)")
axs[1, 1].set_xlabel(r"Signal / VEM (log$_{10}$ scale)")
axs[1, 1].set_ylabel(r"Count")

axs[2, 0].hist(flattenedZen, bins=80, histtype="step", color="blue", alpha=1)
axs[2, 0].set_title(r"Zenith angle")
axs[2, 0].set_xlabel(r"$\theta$")
axs[2, 0].set_ylabel(r"Count")

axs[2, 1].hist(flattenedDist, bins=80, histtype="step", color="blue", alpha=1)
axs[2, 1].set_title(r"Distance")
axs[2, 1].set_xlabel(r"Distance from the shower core / m")
...
#     plt.scatter(avg_distances, time_difference, alpha=0.7, color='cornflowerblue')
